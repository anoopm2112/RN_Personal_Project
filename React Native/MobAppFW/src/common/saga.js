...
export function* handleAPIRequest(apiFn, ...requestData) {
    let { api, endpoint, payload, showErrorToast } = apiFn(...requestData);
    return yield call(invokeApi, api, endpoint, payload, showErrorToast);
}

function* invokeApi(api, endpoint, payload, showErrorToast = true) {
